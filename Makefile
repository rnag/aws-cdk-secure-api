...
lint: ## check style with flake8 and pylint
	flake8 aws_cdk_secure_api tests
	pylint aws_cdk_secure_api tests

test: ## run unit tests quickly with the default Python
	pytest -v --cov=aws_cdk_secure_api --cov-report=term-missing tests/unit

test-all: ## run tests on every Python version with tox
	tox

coverage: ## check code coverage with unit tests quickly with the default Python
	coverage run --source aws_cdk_secure_api -m pytest tests/unit
	coverage report -m
	coverage html
	$(BROWSER) htmlcov/index.html

docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/aws_cdk_secure_api.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ aws_cdk_secure_api
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
